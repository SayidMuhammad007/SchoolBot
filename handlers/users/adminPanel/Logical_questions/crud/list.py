...
@dp.callback_query_handler(lambda c: c.data in ['prevLogic', 'nextLogic'])
async def process_callback(callback_query: types.CallbackQuery, state: FSMContext):
    global current_page
    global data  # Use the global data variable
    if callback_query.data == 'prevLogic':
        current_page = max(0, current_page - 1)
    elif callback_query.data == 'nextLogic':
...
def check(start_idx, end_idx):
    ...
    for i in range(start_idx, end_idx):
        ...
        order_id = data[i][0]
        print(order_id)
        callback_data = f"Logic_{order_id}"
        markup.insert(InlineKeyboardButton(d, callback_data=callback_data))

    # Add previous and next buttons if available
    if has_previous_page:
        prev_button = InlineKeyboardButton(text.prev, callback_data="prevLogic")
        markup.add(prev_button)

    if has_next_page:
        next_button = InlineKeyboardButton(text.next, callback_data="nextLogic")
        if has_previous_page:
