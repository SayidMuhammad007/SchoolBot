...
@dp.callback_query_handler(lambda c: c.data in ['prevGrade', 'nextGrade'])
async def process_callback(callback_query: types.CallbackQuery, state: FSMContext):
    global current_page
    global data  # Use the global data variable
    if callback_query.data == 'prevGrade':
        current_page = max(0, current_page - 1)
    elif callback_query.data == 'nextGrade':
...
def check(start_idx, end_idx):
    ...
    for i in range(start_idx, end_idx):
        ...
        order_id = data[i][0]
        print(order_id)
        callback_data = f"GradeList_{order_id}"
        markup.insert(InlineKeyboardButton(d, callback_data=callback_data))

    # Add previous and next buttons if available
    if has_previous_page:
        prev_button = InlineKeyboardButton(text.prev, callback_data="prevGrade")
        markup.add(prev_button)

    if has_next_page:
        next_button = InlineKeyboardButton(text.next, callback_data="nextGrade")
        if has_previous_page:
