...
current_page = 0
# Echo bot
async def devBookList(message):
    global data  # Use the global data variable
    global current_page
    data = db.selectAll("bookDev")
...
@dp.callback_query_handler(lambda c: c.data in ['prevDevBookList', 'nextDevBookList'])
async def process_callback(callback_query: types.CallbackQuery):
    global current_page
    global data  # Use the global data variable
    if callback_query.data == 'prevDevBookList':
        current_page = max(0, current_page - 1)
    elif callback_query.data == 'nextDevBookList':
...
def check(start_idx, end_idx):
    ...
    if has_previous_page:
        prev_button = InlineKeyboardButton(text.prev, callback_data="prevDevBookList")
        markup.add(prev_button)

    if has_next_page:
        next_button = InlineKeyboardButton(text.next, callback_data="nextDevBookList")
        if has_previous_page:
